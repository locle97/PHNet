...
def get_image_gradients(image):  # -> tuple:
...
def get_4dim_image_gradients(image):  # -> tuple[Any, Any, Tensor, Any]:
    # Return tensors with same size as original image
...
def psnr_loss(
    input: torch.Tensor, target: torch.Tensor, max_val: float
) -> torch.Tensor:
...
class PSNRLossPositive(nn.Module):
    r"""Create a criterion that calculates the PSNR loss.
    Args:
        max_val: The maximum value in the input tensor.
    ...
    def __init__(self, max_val: float, eps: float = 1e-7) -> None:
        super().__init__()
        self.max_val: float = max_val
        self.eps: float = eps
...
def calculate_sad_mse_mad(predict_old, alpha, trimap):
    ...
    predict[trimap == 255] = 1.0
    predict[trimap == 0] = 0.0
    sad_diff = np.sum(np.abs(predict - alpha)) / 1000
    if pixel == 0:
        pixel = (
            trimap.shape[0] * trimap.shape[1]
            - float((trimap == 255).sum())
            - float((trimap == 0).sum())
        )
...
def calculate_sad_fgbg(predict, alpha, trimap):
    ...
    weight_fg[trimap == 255] = 1.0
    weight_bg[trimap == 0] = 1.0
...
class MSE(nn.Module):
    ...
    def forward(self, pred: torch.tensor, target: torch.tensor) -> float:
        ...
        Returns:
            loss (float): Значение MSE.
        """
        target = target.view(pred.size())
        loss = torch.mean(
            (pred - target) ** 2, dim=get_dims_with_exclusion(target.dim(), 0)
        )
        return loss


class FnMSE(nn.Module):
    """Foreground-normalized MSE.
    https://github.com/SamsungLabs/image_harmonization/blob/master/iharm/model/losses.py
    """
    ...
    def forward(
        ...
        loss = (pred - target) ** 2
        delimeter = pred.size(1) * torch.clamp_min(
            torch.sum(mask, dim=reduce_dims), self.min_area
        )
