...
class PatchedHarmonizer(nn.Module):
    def __init__(self, grid_count=1, init_weights=[0.9, 0.1]):
        ...
        self.grid_weights = torch.nn.Parameter(torch.FloatTensor(init_weights), requires_grad=True)
    ...
    def normalize_fg(self, value):
        zeroed_mean = value - (self.fg_local_mean * self.grid_weights[None, None, :, :, None, None]).sum().squeeze()
        # (fg_v * div_global_v +  (1-fg_v) * div_v)
        scaled_var = zeroed_mean * (self.bg_global_var/(self.fg_global_var + self.eps))
        normalized_lg = scaled_var + (self.bg_local_mean * self.grid_weights[None, None, :, :, None, None]).sum().squeeze()     
...
class PatchNormalizer(nn.Module):
    def __init__(self, in_channels=3, eps=1e-7, grid_count=1, weights=[0.5, 0.5], init_value=1e-2):
        ...
        self.fg_var = nn.Parameter(init_value * torch.ones(in_channels)[None, :, None, None], requires_grad=True)
        self.fg_bias = nn.Parameter(init_value * torch.zeros(in_channels)[None, :, None, None], requires_grad=True)
        self.patched_fg_var = nn.Parameter(init_value * torch.ones(in_channels)[None, :, None, None], requires_grad=True)
        self.patched_fg_bias = nn.Parameter(init_value * torch.zeros(in_channels)[None, :, None, None], requires_grad=True)
        self.bg_var = nn.Parameter(init_value * torch.ones(in_channels)[None, :, None, None], requires_grad=True)
        self.bg_bias = nn.Parameter(init_value * torch.zeros(in_channels)[None, :, None, None], requires_grad=True)
        self.grid_weights = torch.nn.Parameter(torch.ones((in_channels, grid_count, grid_count))[None, :, :, :] / (grid_count*grid_count*in_channels), requires_grad=True)
    ...
    def forward(self, fg, bg, mask):
        ...
        self.local_means, self.local_vars = self.compute_patch_statistics(bg, (1-mask))
        ...
        mean_patched_back = (self.local_means.permute(2,3,0,1)*self.grid_weights).sum(dim=[2,3])[:,:,None,None]
        ...
        fg_patch_normalized = patch_normalized * self.patched_fg_var + self.patched_fg_bias

        fg_result = self.weights[0] * fg_normalized + self.weights[1] * fg_patch_normalized
